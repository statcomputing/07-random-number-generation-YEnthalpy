...
r.mixgam <- function(n, theta, control = list(maxit = 1000, 
                                              tol = 1e-5, init = 1)) {
...
logpost <- function(theta, x) {
  ...
  y <- sum(log(delta * dnorm(x, mu1, sqrt(sigma2.1)) + (1 - delta) * dnorm(x, mu2, sqrt(sigma2.2))))
